...
function initSplitResizeJQ(options) {
  ...
  try {
    ...
    if (saved && !isNaN(parseFloat(saved))) {
      ...
      $split[0].style.setProperty('--left-w', opt.baseLeftWidth + 'px');
    }
  } catch (e) {
    $split[0].style.setProperty('--left-w', opt.baseLeftWidth + 'px');
  }
  ...
  function clampWidth(newW) {
    ...
    if (maxLeft < opt.minLeft) {
      maxLeft = opt.minLeft;
    }

    var w = Math.round(newW);

    if (w < opt.minLeft) return opt.minLeft;
  ...
  function stopResize() {
    ...
    if (moved && lastW != null) {
      try {
        localStorage.setItem(opt.storageKey, String(lastW));
      } catch (e) {}
    }
  }

  /* ✅ 화면 줄어들면 현재 폭도 다시 보정 */
  function normalizeCurrentWidth() {
    var cur = parseFloat(getComputedStyle($split[0]).getPropertyValue('--left-w'));
    if (isNaN(cur)) cur = opt.baseLeftWidth;

    var next = clampWidth(cur);
    $split[0].style.setProperty('--left-w', next + 'px');
  }
  ...
  $resizer.on('dblclick.splitResize', function (e) {
    ...
    try {
      localStorage.setItem(opt.storageKey, String(base));
    } catch (err) {}
  ...
  $(window).on('resize.splitResize', function () {
    normalizeCurrentWidth();
  });
